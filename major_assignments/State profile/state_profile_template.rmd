...
In LA, counties are known as parishes. The parishes with the highest number of loans per capita were St. Helena and East Carroll parish. St. Helena is part of the Baton Rouge area. It is relatively small area. East Carroll is another small parish with less than 10,000 people. It is predominantly agriculturaland located in the MI delta area. 
```{r}
...
ggplot() + 
  geom_sf(data=joined_la_zip, aes(fill = abs(state_difference))) + 
  theme_minimal() +
  scale_fill_viridis_b(option="magma",trans = "log") 
...
#having a very hard time with the graphics - https://app.datawrapper.de/chart/9tIgn/publish
```
...
state_naics <- read_csv("census.txt") %>%
 filter(fipstate == "22") %>%
  mutate(short_naics = str_sub(naics, start=1L, end=2L)) %>%
    group_by(short_naics) %>%
  summarise(total=n()) %>%
  arrange(desc(total))
