...
**Answer** It shows a dataframe with state names, how they voted and information about PPP loans. There are 51 rows.
```{r}
...
**Answer** There is not a super clear pattern, but the line shows that the dots are moving from the lower left to the upper right. It shows that there is some relationship between the variables. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** There is a weaker correlation in this one - the line is not as clearly sloped one way or the other. It would be hard to tell if there is a definite relationship. 
```{r}
...
**Answer** a dataframe with three rows showing the R values of buisneses recieving PPP loans and percent per employee. It shows how much they are correlated. The buisinesses recieving ppp had a stronger relationship (.5) than the ppp amount per employee (.2).
```{r}
reveal_data %>%
...
**Answer** This tries to see if the percent of trump vote is significantly correlated to the businesses receiving ppp. This output shows a very small p value,  meaning that there is less likely this is chance. So you can say the correlation has a higher change of being statistically significant. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** the p value here is much larger, meaning we can't for sure say this is statistically significant - it could be a random chance. 
```{r}
...
**Answer** this is a dataframe with 2 rows and 3 colums showing what initially seems like a signifiant difference between Trump/Clinton-voting areas and how many loans they recieved. It seems like Trump areas had more businesses recieving ppp loans than Clinton areas. We have to conduct more testing to see if it is significant. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** It does show there may be a correlation but we should not completely rely on it. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** A higher T and lower p show that it might be significant but we can't say causation. 
```{r}
