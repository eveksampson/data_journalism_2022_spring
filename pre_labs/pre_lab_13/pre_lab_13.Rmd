...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** There is a weaker correlation in this one. 
```{r}
...
**Answer** a dataframe with three rows showing the R values of buisneses recieving PPP loans and percent per employee. It shows how much they are correlated. 
```{r}
reveal_data %>%
...
**Answer** This tries to see if the percent of trump vote is significantly correlated to the businesses recieving ppp. This output shows a very small p value, which is tiny, meaning that there is less likely this is chance. So you can say it may be statistically significant. 
```{r}
...
**Answer** this is a dataframe with 2 rows and 3 colums, we have to condunt more to see if it is significant. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
