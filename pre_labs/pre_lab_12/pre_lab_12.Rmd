...
**Answer** The output is a dataframe with over 11,000 rows of data called "county_vaccinations_by_date" that shows how many doses per county (1st dose, 2nd dose, etc) and dates. 
...
**Answer** We have two extra rows - an unknown row and an "NA" row. 
...
**Answer** Prince George's County never (fully) vaccinated more than 6,900 people per day - that seems to be their max capacity. Most days never got close to that number. 
...
**Answer** Lots of vaccinations through the end of April, then the number decreases pretty sharply. 
...
**Answer**
this is a line-style graph of data in each county over time.
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
this shows the same data but in a log scale graph.
```{r}
...
**Answer** a color-coded map of the density of covid cases by county throughout the state of Maryland.
