...
**Answer** This is the US Bureau of Labor Statistics Current Employment Statistics page. It describes different NAICS codes, systems and changes. It details NAICS sectors, divisions, etc. 
...
**Answer** It's an html output page with the html coding that creates the webpage. It has tags, a head, body, etc. 
...
**Answer** It has an HTML document output showing a head, body and other html characters like tags. It looks like what you'd see when looking at the page source of a webpage.
...
**Answer** You see a list of bullets. The first one is <html> and then there are <head> <body> and (xml attributes) sub-bullets.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** In my Mac, the "inspect" command opens up the HTML coding on the left hand side. You can scroll through the webpage and it will show you exactly what html codes go to what on the website. When you scroll over "Sectors" the html code reads <span class="tableTitle">Table 2. NAICS Sectors</span> ==$0
...
**Answer** The output gives us one R console output and 6 tibbles
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** It is a bullet list. The 6 tibbles are listed one beneath the other under <html>
...
**Answer** It shows a table with Conversions, Reference Month Implemented and Date released. It has four rows. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** This one shows different sectors and descriptions. There are 21 rows, but the last one is not a sector (it's a URL). 
...
**Answer** It shows the same table desribed above exclusively. The others are no longer options. 
...
**Answer** It removes the last row that had the URL (slice) and cleans the names.
...
**Answer** It gives us a table with 20 rows and 2 columns showing NAICS sectors and descriptions.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It's the website for the NAICS 22 utilities sector. It has statistics about the sector.
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** It shows html code with a <head> and <body>
...
**Answer** It shows HTML code for the table with an id of "iag22emp1" with caption, thread, tbody and tfoot. 
...
**Answer** The outout is a 6x6 tibble with 5 rows of data and a 6th row that is for footnotes.
```{r}
...
**Answer** It becomes a 1x2 tibble for just Employment, seasonally adjusted, for November 2021. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This page is for NAICS code 21, mining quarrying and oil and gas extraction. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This is the page for NAICS code 23, Construction. 
...
**Answer** It is a bulleted [1] list of the words. 
```{r}
...
**Answer** It does not work and says "Error in sector_list: object 'sector_list' not found.
```{r}
...
**Answer** It is a [1] list of the sectors requested.
```{r}
...
**Answer** It doesn't work - the same error as before is shown (object: 'sector_list' not found).
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** The code works - you get a list of requested sectors.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** It gives the same error as before (object 'sector_name' not found)
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** It gives a list of 1 through 10
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** It's a 20x2 tibble with a numerical sector and corresponding title or "description"
...
**Answer** it says 20 (meaning 20 rows in that column)
...
**Answer** 20 bullets that are numbers 1-20
...
**Answer** it's a 1x2 tibble with Employees, seasonally adjusted, and the number for November 2021. 
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** It shows a 20x2 tibble with sector numbers in one column and descriptions in the other.
```{r}
# Define url of page we want to scrape
...
**Answer** it's a 20x3 tibble - the same as the previous one but with an additional column with the corresponding url.
...
**Answer** It is a 20x4 tibble - the 4th column is the id tag we got from looking at the html code one each URL for each table for the sector used. 
...
**Answer** this removed the public administration sector, resulting in 19 rows instead of 20.
...
**Answer** It produces 19 individual tibles based on the data.
...
**Answer** It looks like HTML code for each row with xpaths and URLs.
```{r}
...
**Answer** The output is 21 tibbles and the R Console output.
...
**Answer** Now there are 19 tibbles - or dataframes.
...
**Answer** there are now 19 tibbles with the additional information - the row at the bottom shows the additional information presented. 
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** this removed a comma and changed jul_2021 to a number. It also removed everything but the sector name, month/year and number of employees. 
...
**Answer** this is a cleaned tibble with sectors as characters, descriptions as characters and employee numbers
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This is the mining quarrying and oil and gas extraction for NAICS 21.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This is the page for agriculture, forestry, fishing and hunting (NAICS 11)
...
**Answer** The table is clean and all numbers look like they can be validated. 
