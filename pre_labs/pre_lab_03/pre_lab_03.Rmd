...
**Answer** It says "Warning: One or more parsing issues, see problems() for details"
...
**Answer** The output table showed 87,927 problematic rows.
...
**Answer** It does not show any errors.
...
**Answer** The "amount" is "dbl" the "name" field is "chr" and the id column is "dbl"
```{r}
...
```{r}
ppp_maryland_loans <- ppp_maryland_loans %>% #save the thing to itself so it replaces it in its entirety 
...
**Answer**: 192,959 where the franchise is "NA", 275 where the name is "Subway"
...
**Answer**: 135,073; yes, the numbers are the same. 
...
**Answer**:135,073; they are the same records. 
...
**Answer** There are no loans listed within the data for October 2020. My description would be that it makes the "floor" date the first of the month so that it can count the number of loans within the month. It then sorts and summarizes by month.
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan is for 10,000,000 dollars and the minimum amount is 6 dollars. 10 million is not a suspicious amount as that is the maximum loan amount. 6 dollars is vaguely suspicious as I can't imagine a paycheck being 6 dollars but I'm not sure 6 dollars is worth the time/effort of figuring out if it is fraudulent. 
```{r}
...
**Answer** There are 2 loans under 100 dollars. 
```{r}
...
**Answer** 1. The states and street addresses have inconsistent capitalization and wording 2. There are two rows of "Stuart Jones LLC" that are duplicates 3. the title "street addresses" needs and underscore to say "street_addresses." A bonus problem is the amount is classified as <chr>
...
**Answer** This code changes 1_id to x1_id. 
...
**Answer** It says there is an invalid 'type' which I believe means <chr> when it should be a number. You can't have the sum of a character.
...
**Answer** There is one duplicate row of Stuart Jones LLC (meaning Stuart Jones LLC appears twice when it should only appear once).
...
**Answer** 5. Descriptor: eliminates duplicate rows
...
**Answer** Descriptor: This code standardizes a number to specific number of digits.
