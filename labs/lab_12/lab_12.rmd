...
**A1.** "The most recent police incident was a VEHICLE ACCIDENT on 2022-04-19 there were 9 of these incidents in the last 30 days." 

I could also ask how many times a certain type of incident recieved a certain outcome like "cleared," "resolved," etc. Have all vehicle accidents in the last 30 days been closed? 

```{r}
crime_log <- st_read("https://raw.githubusercontent.com/dwillis/wvu-projects/master/crime-log/crime_log.csv") %>%
  clean_names() %>%
...
thirty_days <- crime_log %>% filter(title == latest_date$title)  %>% filter(date > latest_date$date - 30) %>% summarise(count=n())

q1_sentence <- paste("The most recent police incident was a", format(latest_date$title), "on",  format(latest_date$date), "and there were", format(thirty_days$count), "of these incidents in the last 30 days.")
...
**A2.**
The top 3 zip codes from Maryland were 20906 (Aspenville, Montgomery County), 20878 (Gaithersburg, Montgomery County) and 20874 (Germantown, Montgomery County). All of the top Maryland zip codes were in Montgomery County. The top 3 non-Maryland zip codes were 20011 (DC), 20002 (DC) and 20009 (DC) - all DC zip codes. 


```{r}
md_vaccines <- read_csv("https://raw.githubusercontent.com/state-of-maryland/VaccineCSVs/master/Vax_ZIP_CODE_FirstSecondSingle_Totals_Raw.csv") %>%
  clean_names() %>%
  mutate(date = today()) %>%
  filter(fully_vaccinated!='') %>%
  filter(str_length(zip_code) >= 5) 

  
 md_zips <- md_vaccines %>%
   filter(zip_code %in% (20588:21930)) %>%
   arrange(desc(fully_vaccinated)) %>%
   slice(1:25)


q2_sentence <- gsub("-", "", paste("maryland_zips_", format(today()), ".csv", sep="")) 

write_csv(md_zips, q2_sentence)


 non_md_zips <- md_vaccines %>%
   filter(zip_code <20588 | zip_code >21930) %>% 
   arrange(desc(fully_vaccinated)) %>%
   slice(1:25)
 
q2_sentence2 <- gsub("-", "", paste("non_maryland_zips_", format(today()), ".csv", sep="")) 

write_csv(md_zips, q2_sentence2)
...
Calculate the percentage of fully vaccinated for each zip code and then make a map showing the zip codes with less than 50 percent fully vaccinated. 
...
**A3.** The small wedge between WV and PA seems to be the least vaccinated as well as the Delaware edge. Charles and Calvert counties also seem to have lower levels of vaccination

```{r}
q_3 <- md_vaccines %>%
  filter(zip_code %in% (20588:21930))

md <- get_acs(geography = "zip code tabulation area",
...
joined_md <- md %>% left_join(q_3, by=c('GEOID'='zip_code')) %>%
...
ggplot() + 
  ...
  geom_sf(data=joined_md, aes(fill=percent_vaxxed)) + 
  theme_minimal() +
