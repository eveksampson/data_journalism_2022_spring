...
**A1.**
https://datawrapper.dwcdn.net/t3Rhc/1/

Illinios has the most total barber shop PPP loans AND the most undisbursed barber shop PPP loans, with 1713 undisbursed. 
```{r}
#Use the PPP loan data, which is in the data folder and is called `barber_shops_ppp.csv`
barbershop_ppp <- read_csv("data/barber_shops_ppp.csv")

#applications per state
per_state_barbershop <- barbershop_ppp %>%
    group_by(state) %>%
    summarise(count=n()) %>%
    arrange(desc(count))

#one with a total number of applications per state where the undisbursed amount is more than 0.
undispersed_barber_ppp <- barbershop_ppp %>%
...
#Join those dataframes, specifying the join column, and rename the totals columns so that they better describe the values in them.
joined_barbershop <- per_state_barbershop %>%
  left_join(undispersed_barber_ppp, by=c('state'='state')) %>%
  rename(total_barbershop_loans = count.x, total_undispersed_loans = count.y)

#Export the top 10 states (in terms of total number of applications) to a CSV file using `write_csv`
top_ten <- joined_barbershop %>% 
  head(10) 

write_csv(top_ten, "top_ten.csv")

#Using the top 10 states, make a bar chart with Datawrapper. What kind of bar chart is up to you, so experiment with a few kinds of bar charts to see what answers the question above in the clearest way.
...
https://datawrapper.dwcdn.net/P4397/1/

I created a cholopleth map of the US broken up by state. It shows the percentage (code below) of PPP loans for barber shops in each state that remain undisbursed is color-coded accordingly. This is different from the first one because 1) it's a map, and 2) it shows percentages opposed to totals. This also shows results for all states instead of only the top ten.

```{r}
percent_joined_barbershop <- joined_barbershop %>%
  mutate(percent_undisbursed = (total_undispersed_loans/total_barbershop_loans)*100)

write_csv(percent_joined_barbershop, "question_2.csv")
```
...
**A3.**  https://datawrapper.dwcdn.net/nclFZ/1/ 
Kabbage loans were MUCH more common in southern Florida than seemingly anywhere else in the U.S.Kabbage loans were also relatively common in southern Alabama, New Jersey and northern Alaska - three other states that upon first glance, have seemingly little in common. None of the areas aforementioned states seem to be lucrative farming areas as they are in coastal areas or extremely northern (in the case of Alaska). The article elaborates on how potato farms were listed in unlikely areas in southern Florida. In Florida, there are no counties without data, however other states have MANY counties without data shown. There is no data displayed there either because there were no Kabbage loans listed for those counties so when the data was joined, we lost those counties. There were 2,046 counties with Kabbage loans but 3,229 counties overall. So we lost over 1,000 counties and thus there is no data shown there. 

```{r}
#Load the `kabbage_counties.csv` and `all_counties.csv` files in the data folder.
kabbage_lending <- read_csv("data/kabbage_counties.csv")
