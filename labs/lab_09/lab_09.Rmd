...
#filter data to ONLY April and then summarize # of loans per day
question_1_data <- data %>%
  filter(date_approved >= as.Date("2020-04-01"), date_approved <=as.Date("2020-04-30") ) %>%
...
question_1_data %>%
  ...
  labs(
    title="West Virginia PPP Loans in April",
    x = "Date",
    y = "Total Loans",
...
```
**A1.** April 28, 2020 had the most loans.
...
**A2.** 
Nebraska, GA, Iowa, Illinois, Louisiana, Florida, Mississippi, Montana, Wyoming, Kansas
```{r}
#read in data
data_question_2 <- read_csv("data/state_ppp_all.csv")

#change to South and North Dakota
data_question_2 %>% mutate(state= case_when(state== "N Dakota" ~ "North Dakota", state=="S Dakota" ~ "South Dakota", TRUE ~ state))

#census
all_acs <- get_acs(geography = "state", variables = c(total_pop = "B01001_001"), year = 2019)

#join
joined_question_2 <- all_acs %>% 
  inner_join(data_question_2, by=c("NAME" = "state")) %>% 
  mutate(loans_100k = total_ppp_loans/estimate*100000) %>% arrange(desc(loans_100k)) %>% head(10)

#ggplot
joined_question_2 %>%
  ggplot() +
  geom_bar(aes(x=reorder(NAME, loans_100k), weight=loans_100k), fill="blue") +
  coord_flip() +
  labs(
    title="States with highest PPP loans per 100k people",
    x="State",
    y="Loans per 100k people",
    caption="source: SBA PPP loan and U.S. Census Data"
  )
...
**A3.**  According to the scatterplot, it looks like most people had at least the vast majority of their loans forgiven. As the difference between loan amount and amount forgiven increases, we see fewer data points, meaning fewer people had a small percentage of their loan forgiven. 
```{r}

ppp_wv <- read_rds("data/lab_09.rds")

ppp_wv <- ppp_wv %>% 
  filter(amount < 25000) %>% mutate(forgiveness_amount = case_when(is.na(forgiveness_amount)~ 0, TRUE ~ forgiveness_amount))
